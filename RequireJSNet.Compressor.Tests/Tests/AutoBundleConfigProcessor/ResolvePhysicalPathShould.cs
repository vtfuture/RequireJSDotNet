...
{
    public class ResolvePhysicalPathShould
    {
        readonly string projectRootPath;
        readonly RequireJsNet.Compressor.RequireProcessing.AutoBundleConfigProcessor compressor;

        public ResolvePhysicalPathShould()
        {
            var executingDirectory = Directory.GetCurrentDirectory();
            var directory = Path.Combine(executingDirectory, @"..\..\TestData\ResolvePhysicalPathShould");
            projectRootPath = Path.GetFullPath(directory);
            compressor = new RequireJsNet.Compressor.RequireProcessing.AutoBundleConfigProcessor(projectRootPath, "", "", null, System.Text.Encoding.UTF8);
        }

        [Fact]
        public void ResolveUrlWithExtensionRelativeToCurrentModule()
        {
            var relativeModuleUrl = "abc.js";
            var directory = Path.Combine(projectRootPath, "Scripts", "subfolder");
            var expected = Path.Combine(directory, relativeModuleUrl);
        ...
        [Fact]
        public void NotResolveExternalUrls()
        {
            var relativeModuleUrl = "http://a.b.com/a.js";
            var directory = Path.Combine(projectRootPath, "Scripts", "subfolder");

            var actual = compressor.ResolvePhysicalPath(relativeModuleUrl);

            Assert.Null(actual);
        }

        //[Fact]
        //public void ResolveRelativePathsFromSelectedDirectory()
        //{
        //    var expected = "http://a.b.com/a.js";

        //    var actual = compressor.ResolvePhysicalPath("./sub/file", "");
